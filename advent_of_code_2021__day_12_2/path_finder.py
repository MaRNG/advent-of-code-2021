class PathFinder:
    ...
    def find_path(self, current: str, path: [str]):
        ...
        can_visit_small_cave = True

        for cell_in_path in path:
            if cell_in_path.islower() and path.count(cell_in_path) > 1:
                can_visit_small_cave = False

        target_vertex: str

        for target_vertex in self.map_data[current]:
            if target_vertex != PathFinder.START and (not target_vertex.islower() or target_vertex not in path or can_visit_small_cave):
